...
// ════════════════════════════════════════════════════════════════
server.tool("analyze_project", "프로젝트 폴더를 분석하여 기술 스택, 의존성, 기능, 부족한 부분을 파악합니다.", {
    ...
    try {
        ...
        return {
            content: [{ type: "text", text: `❌ 분석 실패: ${err.message}` }],
            isError: true,
        };
    }
});
...
// ════════════════════════════════════════════════════════════════
server.tool("find_repos_for_project", "프로젝트 경로를 입력하면 분석 후 관련 GitHub 저장소를 자동으로 찾아 추천합니다.", {
    ...
    try {
        ...
        return {
            content: [{ type: "text", text: `❌ 실패: ${err.message}` }],
            isError: true,
        };
    }
});
...
// ════════════════════════════════════════════════════════════════
server.tool("search_github_repos", "직접 쿼리를 입력하여 GitHub 저장소를 검색합니다.", {
    ...
    try {
        ...
        const lines = [
            ...
            ``,
            ...repos.map((r, i) => `**${i + 1}. [${r.name}](${r.url})**  ⭐️${r.stars}\n${r.description}`)
        ];
        return { content: [{ type: "text", text: lines.join("\n\n") }] };
    }
    catch (err) {
...
// ════════════════════════════════════════════════════════════════
server.tool("get_repo_detail", "특정 GitHub 저장소의 README와 상세 정보를 가져옵니다.", {
